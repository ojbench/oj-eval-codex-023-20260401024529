...
template <typename Ans, typename Test>
void randnumFill(Ans &ans, Test &test, int n = 200000) {
    for (int i = 0; i < n; i++) {
        int x = randnum();
        if (randnum() % 2) {
            ans.push_back(x);
...
bool iteratorTest() {
    ...
    // iter + n,  iter - n
    for (int i = 0; i < MAX_N; i++) {
        std::deque<int>::iterator ansIter2[] = { ans.begin(), ans.end() };
        sjtu::deque<int>::iterator myIter2[]  = { deq.begin(), deq.end() };

        int offset = randnum() % (ans.size() / 3 + 1);

        ansIter2[0] += offset; ansIter2[1] -= offset;
        myIter2[0]  += offset; myIter2[1]  -= offset;

        for (int j = 0; j < 2; j++)
            if (*ansIter2[j] != *myIter2[j])
                return false;

        if (ansIter2[1] - ansIter2[0] != myIter2[1] - myIter2[0])
            return false;
    }
    ...
    for (int i = 0; i < MAX_N * 2; i++) {
        int offset = randnum() % (ans.size() / 2 + 1);
        switch (randnum() % 4) {
            case 0:  // iter += n
                if (index + offset < (int)ans.size()) {
                    index += offset;
...
bool RobustnessTest() {
    ...
    for (int i = 0; i < 100; i++) deq.push_back(i);
    deq2 = deq;

    try { deq.begin() - deq.begin(); }  catch (...) { ct--; }
    try { deq.end() - deq.end(); }      catch (...) { ct--; }
    try { deq.end() - deq.begin(); }    catch (...) { ct--; }
    try { deq2.begin() - deq.begin(); } catch (...) { ct++; }

    try { deq.insert(deq.begin(), 1); }  catch (...) { ct--; }
    try { deq2.insert(deq.begin(), 1); } catch (...) { ct++; }
    try { deq.erase(deq.begin()); }      catch (...) { ct--; }
    try { deq2.erase(deq.begin()); }     catch (...) { ct++; }

    sjtu::deque<int>::iterator it = deq.begin();
    try { deq.erase(it); }   catch (...) { ct--; }
    try { deq2.erase(it); }  catch (...) { ct++; }

    it = deq.begin();
    try { deq.insert(it, 1); }   catch (...) { ct--; }
    try { deq2.insert(it, 1); }  catch (...) { ct++; }

    try { deq.front(); }   catch (...) { ct--; }
    try { deq.back(); }    catch (...) { ct--; }
    try {
        for (int i = 0; i < 100; i++) deq.pop_back();
        deq.pop_back();
        ct++;
    } catch (...) { ct--; }
    try { deq.pop_front(); } catch (...) { ct++; }
...
int main() {
    ...
    bool error = false;
    for (int i = 0; i < (int)(sizeof(testFunc) / sizeof(testFunc[0])); i++) {
